...
exports.default = strapi_1.factories.createCoreController('api::imovel.imovel', ({ strapi }) => ({
    async find(ctx) {
        try {
            ...
            console.log(`[Dashboard Filter] Results: ${(results === null || results === void 0 ? void 0 : results.length) || 0}`);
    ...
    async findOne(ctx) {
        ...
        try {
            ...
            const isOwner = ctx.state.user && propertyAny.usuario && (propertyAny.usuario.documentId === ctx.state.user.documentId ||
                propertyAny.usuario.id === ctx.state.user.id);
            // If not owner and not published, deny
            if (!isOwner && propertyAny.estatus !== 'publicado') {
                console.warn(`[findOne] Unauthorized: ${id} by ${((_a = ctx.state.user) === null || _a === void 0 ? void 0 : _a.username) || 'Guest'}`);
